...
class Network:
    ...
    def feedforward(self, inputs, real_value):
        ...
        preview = ((last_layer_w * inputs.t()) + last_layer_b).matrix
    ...
    def backpropagation(self, inputs, real_value):
        ...
        delta = derivates[-1]
        if self.cost == self.linear_regression:
            nabla_w.append((modules[-1] * delta.matrix[0][0]).t())
            modules.pop()
    ...
    def trainer_weights_stochastic(self, data, responses, eppochs=10, lr=0.1, show=True):
    ...
    def trainer_weights_mini_batch(self, data, responses, len_mini_batch=5,  eppochs=10, lr=0.1, show=True):
        st_nabla_w = [layer * 0 for layer in self.weights]
        st_nabla_b = [layer * 0 for layer in self.biases]
